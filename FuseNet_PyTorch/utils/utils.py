...
def get_scores(conf_mat):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        global_acc = np.diag(conf_mat).sum() / float(conf_mat.sum())
        mean_acc = np.diag(conf_mat) / conf_mat.sum(1).astype(float)
        iou = np.diag(conf_mat) / (conf_mat.sum(1) + conf_mat.sum(0) - np.diag(conf_mat)).astype(float)
